...
class Common
{
    ...
    public function getShortUrl()
    {
        ...
        if ($json = json_decode($ret, true)) {
            if ($json['code'] == 200 && isset($json['data']['short_url'])) {
                return resultJson(1, '生成成功！', ['long_url' => $url, 'short_url' => $json['data']['short_url']]);
            }
        }
        ...
        if ($ret && $json = json_decode($ret, true)) {
            if ($json['ret'] == 0 && isset($json['short_url'])) {
                return resultJson(1, '生成成功！', ['long_url' => $url, 'short_url' => $json['short_url']]);
            }
        }

        return resultJson(1, '接口失效',['long_url' => $url, 'short_url' => $url]);
